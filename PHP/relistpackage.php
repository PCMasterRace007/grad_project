...
$pid = $_GET["pid"];
$sql = "UPDATE packages SET islisted = 1 WHERE pid = '$pid'";
$ret = $db->exec($sql);
if (!$ret) {
    header("Refresh: 2; URL=showlisting.php?pid=$pid");
    echo '<h3 class="w3-center txt">Failed to relist package, try again.</h3>';
} else {
    header("Refresh: 2; URL=showlisting.php?pid=$pid");
    echo '<h3 class="w3-center txt">Successfully relisted package.</h3>';
}