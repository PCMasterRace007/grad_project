...
$db = new MyDB();
if (!$db) {
    header('Refresh: 2; URL=../HTML/bsignup.html');
...
if ($pass != $passconfirm) {
    ...
    if ($len != 10) {
        $db->close();
        header('Refresh: 2; URL=../HTML/bsignup.html');
        echo '<h3 class="txt w3-center">Incorrect number, Please signup with a correct 10 digit number and do not provide country code. Redirecting to signup.</h3>';
        exit();
    } else {
        $res = $db->exec($sql);
        if (!$res) {
